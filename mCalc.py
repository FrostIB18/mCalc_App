...
class mCalcApp(App):
    ...
    def build(self):
        ...
        for row in buttons:
            ...
            for label in row:
                if label == "C":
                    button = Button(text=label, color='red', font_size=30, pos_hint={"center_x": 0.5, "center_y": 0.5},
                                    on_press=self.on_button_press)
                elif label == "<<":
                    button = Button(text=label, color='yellow', font_size=30, pos_hint={"center_x": 0.5, "center_y": 0.5},
                                    on_press=self.on_button_press)
                else:
                    button = Button(text=label, font_size=30, pos_hint={"center_x": 0.5, "center_y": 0.5},
                                    on_press=self.on_button_press)
